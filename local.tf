locals {
  ...
  cluster_security_group_id = element(concat(compact(concat(
    aws_security_group.cluster.*.id,
    list(var.cluster_security_group_id),
  )), list("")), 0)
  ...
  worker_security_group_id = element(concat(compact(concat(
    aws_security_group.workers.*.id,
    list(var.worker_security_group_id),
  )), list("")), 0)
